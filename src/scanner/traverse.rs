use super::crossdev;
use super::get_size_or_panic;
use super::inodefilter::InodeFilter;
use super::walk::{Throttle, WalkOptions};
...
impl BackgroundTraversal {
    pub fn start(
        ...
        let (entry_tx, entry_rx) = crossbeam_channel::bounded(100);
        std::thread::Builder::new()
            .name("rustdirstat-walk".to_string())
            .spawn({
                let walk_options = walk_options.clone();
                let mut io_errors: u64 = 0;
                move || {
                    for root_path in input.into_iter() {
                        tracing::info!("Walking {:?}", root_path);
                    ...
                    let _ = entry_tx.send(TraversalEvent::Finished(io_errors));
                }
            })?;
...
mod tests {
    ...
    fn size_of_entry_data() {
        assert!(
            ...
            "EntryData size ({}) should not exceed 80 bytes",
